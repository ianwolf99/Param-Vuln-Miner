...
run_paramspider() {
  ...
  python3 paramspider.py --domain "$domain" --exclude woff,css,js,png,svg,php,jpg,jpeg,gif --output "output/$domain-urls.txt"

  # Check if output file exists
  if [ ! -f "output/$domain-urls.txt" ]; then
    echo "[!] Error: ParamSpider did not generate the output file for $domain."
    exit 1
  fi
}
...
filter_urls() {
  local domain=$1
  local input_file="output/$domain-urls.txt"  # Fix the path to the correct output location
